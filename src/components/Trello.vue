<template>
	...
	<section class="lists-container pt-3">
		<div class="list" v-for="(list, index) in lists" :key="index">
			...
			<draggable class="list-items" :list="list.items" group="people" tag="ul" ghost-class="ghost" chosen-class="chosen" drag-class="drag" @end="updateItemOrder">
				<li v-for="(element, index) in list.items" :key="index" >
					{{ element.name }} {{ index }}
				</li>
			</draggable>
		</div>
